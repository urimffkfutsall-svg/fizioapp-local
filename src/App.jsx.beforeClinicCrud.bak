...
const SuperadminDashboard = ({ clinics }) => {
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:12}}>
        <div>
          <h1 style={{fontSize:22,fontWeight:700,color:"#1A2332",margin:0}}>Mirëmbrëma, {user.role==="superadmin"?"Urimi":user.role==="admin"?"Admin":"Fizioterapis"} 👋</h1>
            ...
            <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,overflow:"hidden"}}>
              ...
              {[{date:"01 Apr",therapist:"Dr. Morina",s:"Dhimbje e poshtme e shpinës, intensitet 6/10",o:"ROM kufizuar, tensioni muskulor i lartë",a:"Lumbalgjia akute",p:"Termokompresa + masazh 3x/javë"},
                ...
                <div key={i} style={{padding:"14px 18px",borderBottom:"1px solid #F5F7FA"}}>
                  ...
                  {[["S — Subjektive",n.s],["O — Objektive",n.o],["A — Vlerësimi",n.a],["P — Plani",n.p]].map(([l,v])=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:16}}>
        <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,overflow:"hidden"}}>
          <div style={{padding:"14px 18px",borderBottom:"1px solid #E8ECF2"}}>
            <span style={{fontSize:13,fontWeight:600,color:"#1A2332"}}>{dayAppts.length} terme — {activeDay} Prill</span>
          </div>
          {dayAppts.length===0?(
          ...
          <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:16}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                <button style={{width:24,height:24,border:"1px solid #E8ECF2",borderRadius:6,background:"none",cursor:"pointer",fontSize:12}}>‹</button>
...
// ─── SHARED CLINICS STATE (lifted up via props) ───────────────────────────────
...
const ClinicsPage = ({ clinics, setClinics }) => {
  ...
  const handleSubmit=()=>{
    if(!form.name.trim()||!form.city.trim()){showToast("⚠ Plotëso fushat e detyrueshme!");return;}
    ...
    showToast("✓ Ordinanca u regjistrua!");
  };

  const toggleStatus=id=>setClinics(prev=>prev.map(c=>c.id===id?{...c,status:c.status==="active"?"inactive":"active"}:c));

  const addAdmin=()=>{
    if(!adminForm.name||!adminForm.pin){showToast("⚠ Emri dhe PIN janë të detyrueshëm!");return;}
    ...
    showToast("✓ Administratori u shtua me PIN: "+adminForm.pin);
  };
  ...
  const removeAdmin=(clinicId,adminId)=>{
    ...
    showToast("✓ Administratori u largua");
  };
  ...
  const addTreatmentType=()=>{
    ...
    showToast("✓ Lloji i trajtimit u shtua!");
  };
...
<title>Terapi — ${therapy.patient} — ${clinic.name}</title>
<style>
...
<div style="max-width:210mm;margin:0 auto">
  <div class="no-print">
    <button class="btn-p" onclick="window.print()">🖨️ Printo A4</button>
    <button class="btn-c" onclick="window.close()">✕ Mbyll</button>
  </div>
  <div class="header">
  ...
  <div class="info-grid">
    ...
    <div class="info-box"><div class="lbl">Fizioterapis</div><div class="val">${therapy.therapist||"—"}</div></div>
    <div class="info-box"><div class="lbl">Data</div><div class="val">${therapy.date||"—"}</div></div>
    <div class="info-box"><div class="lbl">Kohëzgjatja</div><div class="val">${therapy.duration||45} min</div></div>
    <div class="info-box"><div class="lbl">Statusi</div><div class="val" style="color:#15803D">✓ Kryer</div></div>
  </div>
  ...
  <div class="footer">
    ...
    <span>© Fizioapp 2026 — Konfidencial</span>
  </div>
</div>
</body></html>`);
    w.document.close();
  ...
  if(view==="addTherapy"&&selected) return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        ...
        <h1 style={{fontSize:20,fontWeight:700,color:"#1A2332",margin:0}}>Shto Terapi — {selected.name}</h1>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:20}}>
        <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:28}}>
          ...
          <div style={{display:"flex",gap:10,marginTop:20}}>
            <button onClick={()=>{
              if(!therapyForm.patient||!therapyForm.type){showToast("⚠ Pacienti dhe lloji janë të detyrueshëm!");return;}
              showToast("✓ Terapia u regjistrua!");
            ...
            <button onClick={()=>{
              if(!therapyForm.patient||!therapyForm.type){showToast("⚠ Pacienti dhe lloji janë të detyrueshëm!");return;}
              showToast("✓ Terapia u regjistrua!");setView("detail");
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:14}}>
            <div style={{fontSize:11,fontWeight:700,color:"#92400E",marginBottom:4}}>💡 PDF automatik</div>
  ...
  if(view==="addTreatType"&&selected) return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        <button onClick={()=>setView("detail")} style={{padding:"7px 14px",border:"1px solid #E8ECF2",borderRadius:8,background:"#fff",cursor:"pointer",fontSize:13,color:"#5A6A7E",fontFamily:"'DM Sans',sans-serif"}}>← Kthehu</button>
        <h1 style={{fontSize:20,fontWeight:700,color:"#1A2332",margin:0}}>Lloje Trajtimesh — {selected.name}</h1>
      </div>
  ...
  if(view==="addAdmin"&&selected) return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        <button onClick={()=>setView("detail")} style={{padding:"7px 14px",border:"1px solid #E8ECF2",borderRadius:8,background:"#fff",cursor:"pointer",fontSize:13,color:"#5A6A7E",fontFamily:"'DM Sans',sans-serif"}}>← Kthehu</button>
        <h1 style={{fontSize:20,fontWeight:700,color:"#1A2332",margin:0}}>Shto Administrator — {selected.name}</h1>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:20}}>
        <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:28}}>
          ...
          <div style={{padding:"12px 14px",background:"#EBF4FF",border:"1px solid #BFDBFE",borderRadius:10,marginBottom:20}}>
            <div style={{fontSize:11,fontWeight:700,color:"#2563EB",marginBottom:3}}>📱 Si funksionon kyçja me PIN</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:12,padding:14}}>
            <div style={{fontSize:11,fontWeight:700,color:"#15803D",marginBottom:4}}>✓ Siguri e PIN-it</div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:20,textAlign:"center"}}>
            ...
            <div style={{marginTop:16,borderTop:"1px solid #E8ECF2",paddingTop:14,textAlign:"left"}}>
              {[{l:"Telefon",v:selected.phone||"—"},{l:"Email",v:selected.email||"—"},{l:"Adresa",v:selected.address||"—"},{l:"Aktive nga",v:selected.since}].map(x=>(
          ...
          <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:16}}>
            ...
            {(selected.treatmentTypes||[]).map(t=>(
              <div key={t} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#F8FAFF",borderRadius:7,marginBottom:6}}>
                ...
                <button onClick={()=>removeTreatType(selected.id,t)} style={{border:"none",background:"none",cursor:"pointer",color:"#9DABBE",fontSize:14,lineHeight:1}}>×</button>
              </div>
            ))}
        ...
        <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:24}}>
          ...
          {logoPreview
            ...
            :<div onClick={()=>fileRef.current.click()} style={{border:"2px dashed #BFDBFE",borderRadius:14,padding:"32px 20px",textAlign:"center",cursor:"pointer",background:"#F8FAFF"}}
              ...
              <div style={{fontSize:11,color:"#9DABBE"}}>PNG, JPG, SVG — max 5MB</div>
            </div>}
        </div>
      </div>
    </div>
            ...
            <div style={{padding:"14px 18px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:12}}>
                {[{l:"Pacientë",v:c.patients},{l:"Adminë PIN",v:(c.admins||[]).length},{l:"Të Ardhura",v:`€${c.revenue.toLocaleString()}`}].map(x=>(
                  <div key={x.l}><div style={{fontSize:10,color:"#9DABBE",fontWeight:600,textTransform:"uppercase"}}>{x.l}</div><div style={{fontSize:13,fontWeight:700,color:"#1A2332",marginTop:2}}>{x.v}</div></div>
                ))}
              </div>
...
<head>
<meta charset="UTF-8"/>
<title>Raport — ${clinic.name}</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&display=swap');
  *{margin:0;padding:0;box-sizing:border-box}
...
<div class="page">
  ...
  <div class="no-print">
    <button class="btn-print" onclick="window.print()">🖨️ Printo A4</button>
    <button class="btn-close" onclick="window.close()">✕ Mbyll</button>
    <span class="preview-note">Raport për: <strong>${clinic.name}</strong> — ${clinic.city}</span>
  </div>
  ...
  <div class="title-block">
    <div>
      ...
      <p>${clinic.name} — ${clinic.city} · Aktiv që nga ${clinic.since}</p>
    </div>
  ...
  <!-- KPI -->
  <div class="kpi-grid">
    <div class="kpi"><div class="kpi-icon">👥</div><div class="kpi-val">${st.patients}</div><div class="kpi-label">Pacientë Aktivë</div><div class="kpi-change">↑ +8 vs muajit parë</div></div>
    <div class="kpi"><div class="kpi-icon">📅</div><div class="kpi-val">${st.sessions}</div><div class="kpi-label">Seanca Totale</div><div class="kpi-change">↑ +14 vs muajit parë</div></div>
    <div class="kpi"><div class="kpi-icon">💶</div><div class="kpi-val">€${st.revenue.toLocaleString()}</div><div class="kpi-label">Të Ardhura</div><div class="kpi-change">↑ +8.3% rritje</div></div>
    <div class="kpi"><div class="kpi-icon">✅</div><div class="kpi-val">${st.attendance}%</div><div class="kpi-label">Prezenca</div><div class="kpi-change">↑ +2.1% muajin</div></div>
  </div>

  <!-- SUMMARY -->
  <div class="summary-box">
    <h3>Përmbledhja Financiare — Prill 2026</h3>
  ...
  <!-- CHART -->
  <div class="section">
    <div class="section-title">Grafiku i Të Ardhurave Mujore — 2026</div>
  ...
  <div class="section">
    ...
    <div class="staff-grid">
      ${STAFF.map(s=>{
        ...
        return `<div class="staff-card"><div class="staff-avatar">${initials}</div><div style="flex:1"><div class="staff-name">${s.name}</div><div class="staff-stats">${s.sessions} seanca · ${s.patients} pac. · ${s.attendance}%</div><div class="prog-bar"><div class="prog-fill" style="width:${s.attendance}%"></div></div></div><div class="staff-rating">${s.rating} ★</div></div>`;
      }).join("")}
    </div>
  </div>
  ...
  <div class="footer">
    <div class="footer-left">
      ...
      <div class="f-sub">Raport gjeneruar nga Fizioapp — 01 Prill 2026 në 14:32 · Ref: RPT-${clinic.id}-2026-04</div>
    </div>
    <div class="footer-right">
      <div class="page-num">Faqe 1 / 1</div>
      <div class="confidential">© Fizioapp 2026 — Konfidencial</div>
    </div>
  </div>
  ...
  return (
    <div>
      ...
      <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:20,marginBottom:20}}>
        <div style={{fontSize:13,fontWeight:600,color:"#1A2332",marginBottom:14}}>Printo Raport A4 — Zgjidh Ordinancën</div>
        ...
        {selectedClinic && (
          <div style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:"linear-gradient(135deg,#EBF4FF,#F0F7FF)",borderRadius:10,border:"1px solid #BFDBFE"}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:"#5A6A7E"}}>{selectedClinic.city} · {selectedClinic.phone||"—"} · {selectedClinic.address||"—"}</div>
              <div style={{fontSize:10,color:"#9DABBE",marginTop:1}}>
                {selectedClinic.logo ? "✓ Logo e ngarkuar — do shfaqet në raport" : "⚠ Pa logo — do përdoret shkronjat fillestare"}
              </div>
            </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:20}}>
          <div style={{fontSize:13,fontWeight:600,color:"#1A2332",marginBottom:16}}>Të Ardhurat Mujore — 2026</div>
          <div style={{display:"flex",alignItems:"flex-end",gap:8,height:120}}>
            {months.map((m,i)=>(
              <div key={m} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                ...
                <div style={{fontSize:9,color:i===3?"#2563EB":"#9DABBE",fontWeight:i===3?700:400}}>{m}</div>
              </div>
            ))}
          </div>
        </div>
        <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:20}}>
          <div style={{fontSize:13,fontWeight:600,color:"#1A2332",marginBottom:16}}>Pacientë të Rinj — 2026</div>
          <div style={{display:"flex",alignItems:"flex-end",gap:8,height:120}}>
            {months.map((m,i)=>(
              <div key={m} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
...
export default function FizioApp() {
  ...
  // Superadmin nav — NO patient/appointment/treatment/payment/report items
  const superadminNav=[
  ...
  // Admin/Fizioterapis nav — full clinical access
  const clinicNav=[
  ...
  const TreatmentsPage = () => {
    ...
    const addSession=(id)=>{
      ...
      showToast("✓ Seanca u regjistrua me sukses!");
    };

    const saveSOAP=()=>{
      showToast("✓ Shënimet SOAP u ruajtën!");
      setShowSOAP(false);setSoap({s:"",o:"",a:"",p:""});
    };

    const submitNewPlan=()=>{
      if(!newPlan.patient||!newPlan.therapist){showToast("⚠ Plotëso fushat e detyrueshme!");return;}
      const plan={
      ...
      showToast("✓ Plani i trajtimit u shtua!");
    };
          ...
          {showSOAP&&(
            <div style={{background:"#fff",border:"2px solid #BFDBFE",borderRadius:14,padding:24,marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                <span style={{fontSize:14,fontWeight:600,color:"#1A2332"}}>Shënim SOAP i Ri — {p.patient}</span>
                <button onClick={()=>setShowSOAP(false)} style={{border:"none",background:"none",cursor:"pointer",fontSize:18,color:"#9DABBE"}}>×</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {[{k:"s",l:"S — Subjektive",ph:"Çfarë raporton pacienti? Dhimbje, simptoma..."},
                  {k:"o",l:"O — Objektive",ph:"Gjetjet fizike, matjet, vëzhgimet..."},
                  {k:"a",l:"A — Vlerësimi",ph:"Diagnoza, progresi, interpretimi..."},
                  {k:"p",l:"P — Plani",ph:"Trajtimi i radhës, ushtrimet, rekomandimet..."}].map(f=>(
                  <div key={f.k}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{background:"#fff",border:"1px solid #E8ECF2",borderRadius:14,padding:20}}>
                ...
                {[{l:"Pacienti",v:p.patient},{l:"Lloji",v:p.type},{l:"Fizioterapis",v:p.therapist},
                  {l:"Fillimi",v:p.startDate},{l:"Mbarimi",v:p.endDate||"—"},{l:"Statusi",v:p.status}].map(x=>(
              ...
              {Array.from({length:p.done},(_, i)=>{
                ...
                return (
                  <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:10,background:"#F8FAFF",border:"1px solid #E8ECF2",marginBottom:8}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:500,color:"#1A2332"}}>Seanca {i+1} — {p.type}</div>
  ...
  const SettingsPage = () => {
    ...
    const saveBtn=(label,action)=>(
      <button onClick={()=>{action&&action();showToast("✓ Ndryshimet u ruajtën!");}}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
                    {[{k:"light",l:"E Bardhë",desc:"Minimalist & e pastër",preview:"#FAFAFA"},
                      ...
                      <div key={t.k} style={{border:"2px solid",borderColor:t.k==="blue"?"#2563EB":"#E8ECF2",borderRadius:12,padding:16,cursor:"pointer",textAlign:"center",background:t.k==="blue"?"#EBF4FF":"#fff"}}>
                        ...
                        {t.k==="blue"&&<div style={{fontSize:10,fontWeight:700,color:"#2563EB",marginTop:6}}>✓ AKTIVE</div>}
                      </div>
                    ))}
                  </div>
                </div>
  ...
  const renderPage=()=>{
    if(page==="dashboard") return isSuperadmin
      ? <SuperadminDashboard clinics={clinics}/>
      : <Dashboard user={user}/>;
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:"#fff",borderBottom:"1px solid #E8ECF2",padding:"0 24px",height:58,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontSize:15,fontWeight:600,color:"#1A2332"}}>
            ...
            {!isSuperadmin&&userClinic&&<span style={{fontSize:12,color:"#9DABBE",fontWeight:400,marginLeft:10}}>— {userClinic.name}</span>}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
